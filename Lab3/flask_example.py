...
def weather(domain):
    if weatherCache.get(domain) != None:
        ...
        if js["result"]["addressMatches"] == []:
            # I decided not to cache this response in case weather information does get added at any point in time.
    ...
    if rangeCache.get(domain) != None:
        ...
        for line in splitted:
            if line.startswith("NetRange"):
                netRange = (
                    f"Network range for {domain} is {line.partition("NetRange")[2][1:]}"
                )
